  ...
  const submitName = (e) => {
    ...
    if (filtered.length == 0) {
      phonebookService
        .create(newPerson)
        .then(returnedPerson => {
          setPersons(persons.concat(returnedPerson.id == newPerson.id ? newPerson : returnedPerson))
          setNotifHelper(`${newPerson.name} was added to the phonebook`, false)
        })
        .catch(()  => {
          setNotifHelper(`Failed to add ${newPerson.name} to the phonebook, something went wrong with the server`, true)
        })
